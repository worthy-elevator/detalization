...
<div class="container" style="width: 930px; margin: 0 auto;">
  <div class="container-left" style="float: left;width:640px;">
    <div class="customers-img" style="<?include ('./services/10/fuckinoff.php');?>margin: 50 0">
    <?
$folder = "services/10/";
...
foreach($services as $key=>$name)
{
  $alt= str_replace("~"," ",$name);
          ...
          input+label #'.$key.'
          {
            background-image: url(/services/10/'.$key.');
          }
          </style>
<div class="pics" style="margin:10 15 10 5">
      <div class="image"><input id="'.$key.'1" type="checkbox"><label for="'.$key.'1" onclick="showMenu('.$key.')" style="background-image: url(/services/10/'.$name.');"><img class="fullimg" src="/services/10/'.$name.'" alt="'.$alt.'" id="'.$key.'" style="height:120px;"></label></div>
    </div>
          ';
 }
}
